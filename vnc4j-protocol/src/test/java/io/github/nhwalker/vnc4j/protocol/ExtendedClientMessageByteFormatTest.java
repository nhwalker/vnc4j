...
class ExtendedClientMessageByteFormatTest {
    ...
    // -----------------------------------------------------------------------
    // QEMU Extended Key Event (type 255, sub-type 0)
    // -----------------------------------------------------------------------

    /**
     * Verifies the QEMU Extended Key Event message byte format.
     *
     * <pre>
     * From rfbproto.rst.txt - QEMU Extended Key Event Message:
     *
     *   =============== ==================== ========== =======================
     *   No. of bytes    Type                 [Value]    Description
     *   =============== ==================== ========== =======================
     ...
     *   =============== ==================== ========== =======================
     *
     *   The keysym and down-flag fields take the same values as described for
     *   the KeyEvent message. The keycode is the XT keycode that produced the keysym.
     * </pre>
     */
    @Test
    void testQemuExtendedKeyEvent_byteFormat() throws IOException {
        QemuExtendedKeyEvent msg = QemuExtendedKeyEvent.newBuilder()
                .downFlag(1)
                .keysym(0x41) // 'A'
                .keycode(0x1e) // XT scancode for A
                .build();
        byte[] bytes = serialize(msg::write);

        assertEquals(12, bytes.length, "QemuExtendedKeyEvent must be exactly 12 bytes");
        assertEquals((byte) 255, bytes[0], "message-type must be 255");
        assertEquals((byte) 0, bytes[1], "submessage-type must be 0");
        // down-flag = 1 (big-endian U16)
        assertEquals((byte) 0, bytes[2]);
        assertEquals((byte) 1, bytes[3]);
        // keysym = 0x41 (big-endian U32)
        assertEquals((byte) 0, bytes[4]);
        assertEquals((byte) 0, bytes[5]);
        assertEquals((byte) 0, bytes[6]);
        assertEquals((byte) 0x41, bytes[7]);
        // keycode = 0x1e (big-endian U32)
        assertEquals((byte) 0, bytes[8]);
        assertEquals((byte) 0, bytes[9]);
        assertEquals((byte) 0, bytes[10]);
        assertEquals((byte) 0x1e, bytes[11]);
    }

    // -----------------------------------------------------------------------
    // QEMU Audio Client Messages (type 255, sub-type 1)
    // -----------------------------------------------------------------------

    /**
     * Verifies the QEMU Audio Client Enable message (operation 0).
     *
     * <pre>
     * From rfbproto.rst.txt - QEMU Audio Client Message:
     *
     ...
     *   2               U16                  0          operation (enable)
     *   =============== ==================== ========== =======================
     * </pre>
     */
    @Test
    void testQemuAudioClientEnable_byteFormat() throws IOException {
        QemuAudioClientEnable msg = QemuAudioClientEnable.newBuilder().build();
        byte[] bytes = serialize(msg::write);

        assertEquals(4, bytes.length, "QemuAudioClientEnable must be 4 bytes");
        assertEquals((byte) 255, bytes[0], "message-type must be 255");
        assertEquals((byte) 1, bytes[1], "submessage-type must be 1");
        // operation = 0 (enable) as big-endian U16
        assertEquals((byte) 0, bytes[2]);
        assertEquals((byte) 0, bytes[3]);
    }

    /**
     * Verifies the QEMU Audio Client Disable message (operation 1).
     *
     * <pre>
     * From rfbproto.rst.txt - QEMU Audio Client Message:
     *
     *   The second operation disables audio capture on the server:
     *
     *   =============== ==================== ========== =======================
     *   No. of bytes    Type                 [Value]    Description
     *   =============== ==================== ========== =======================
     *   1               U8                   255        message-type
     *   1               U8                   1          submessage-type
     *   2               U16                  1          operation (disable)
     *   =============== ==================== ========== =======================
     * </pre>
     */
    @Test
    void testQemuAudioClientDisable_byteFormat() throws IOException {
        QemuAudioClientDisable msg = QemuAudioClientDisable.newBuilder().build();
        byte[] bytes = serialize(msg::write);

        assertEquals(4, bytes.length);
        assertEquals((byte) 255, bytes[0]);
        assertEquals((byte) 1, bytes[1]);
        // operation = 1 (disable) as big-endian U16
        assertEquals((byte) 0, bytes[2]);
        assertEquals((byte) 1, bytes[3]);
    }

    /**
     * Verifies the QEMU Audio Client Set Format message (operation 2).
     *
     * <pre>
     * From rfbproto.rst.txt - QEMU Audio Client Message:
     *
     *   The third operation sets the audio sample format:
     *
     *   =============== ==================== ========== =======================
     *   No. of bytes    Type                 [Value]    Description
     *   =============== ==================== ========== =======================
     *   1               U8                   255        message-type
     *   1               U8                   1          submessage-type
     *   2               U16                  2          operation (set format)
     *   1               U8                              sample-format
     *   1               U8                              nchannels
     *   4               U32                             frequency
     *   =============== ==================== ========== =======================
     *
     *   sample-format values: 0=U8, 1=S8, 2=U16, 3=S16, 4=U32, 5=S32
     *   nchannels: 1 (mono) or 2 (stereo)
     * </pre>
     */
    @Test
    void testQemuAudioClientSetFormat_byteFormat() throws IOException {
        QemuAudioClientSetFormat msg = QemuAudioClientSetFormat.newBuilder()
                .sampleFormat(3)   // S16
                .nchannels(2)      // stereo
                .frequency(44100L)
                .build();
        ...
        assertEquals(10, bytes.length, "QemuAudioClientSetFormat must be 10 bytes");
        assertEquals((byte) 255, bytes[0]);
        assertEquals((byte) 1, bytes[1]);
        // operation = 2 (set format) as big-endian U16
        assertEquals((byte) 0, bytes[2]);
        ...
        // frequency = 44100 (0x00 0x00 0xAC 0x44) big-endian U32
        assertEquals((byte) 0x00, bytes[6]);
        assertEquals((byte) 0x00, bytes[7]);
        assertEquals((byte) 0xAC, bytes[8]);
        assertEquals((byte) 0x44, bytes[9]);
    }
}
